...
public:
    ...
    void print() const {
        kprintf("Unknown tag tag = %u, tag_size = %u\n",
                tag, tag_size);
    }
    uint32_t tag_size;
    uint32_t tag;
...
public:
    ...
    void print() const {
        kprintf("Core: writable = %s, pagesize = %#010x, rootdev = %#010x\n",
...
public:
    ...
    void print() const {
        kprintf("Mem: start = %#010x, size = %#010x\n", start, size);
    }
    uint32_t size;
    uint32_t start;
...
public:
    ...
    void print() const {
        kprintf("Ramdisk: load = %s, prompt = %s, size = %#010x, start = %#010x\n",
...
public:
    ...
    void print() const {
        kprintf("Initrd2: start = %010x, size = %#010x\n", start, size);
    }
    uint32_t start;
    uint32_t size;
...
public:
    ...
    void print() const {
        kprintf("VideoLFB: width = %hu, height = %hu, depth = %hu, linelength = %hu, base = %#010x, size = %#x, red_size = %hhu, red_pos = %hhu, green_size = %hhu, green_pos = %hhu, blue_size = %hhu, blue_pos = %hhu, rsvd_size = %hhu, rsvd_pos = %hhu\n",
